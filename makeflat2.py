...
if __name__ == "__main__":
	...
	balance_r =  numpy.mean(median_r) / median_r
	balance_g =  numpy.mean(median_g) / median_g
	balance_b =  numpy.mean(median_b) / median_b

	flat_r = numpy.rint(1/balance_r*255)
	flat_r = numpy.array(flat_r, dtype="uint8")
	flat_r = numpy.reshape(flat_r, (size[1], size[0]))

	flat_g = numpy.rint(1/balance_g*255)
	flat_g = numpy.array(flat_g, dtype="uint8")
	flat_g = numpy.reshape(flat_g, (size[1], size[0]))

	flat_b = numpy.rint(1/balance_b*255)
	...
	flat = PIL.Image.merge("RGB", (imageflat_r, imageflat_g, imageflat_b))
